...
import { Oficina } from '../../../core/services/movOficina.service';
...
import { VentanaVerInformacionMovOficina } from './ventanaVerInformacionMovOficina';
import { VentanaBusquedaMovOficina } from './ventanaBusquedaMovOficina';
import { VentanaEliminaMovOficina } from './ventanaEliminarOficina';
...
export class OficinaComponent {
  ...
  setDataLogin() {
    this.puenteData.disparadorData.emit({ dato: 'Oficina', poisionX: '' })
  }
...
async ActualizarRegistro() {
  ...
  async verDatosoMovInversion(id: number) {
     ...
     const datos = await this.servicio.cargaMovOficinaId( id )
    ...
    if(datos.status === 'error'){
     ...
     const dialogRef = this._dialog.open(VentanaVerInformacionMovOficina, {
       disableClose: true,
       data: datos,
       width: '705px',
       maxWidth: '100%',
  ...
  editaroMovInversion(id: number) {
    ...
    const dialogRef = this._dialog.open(VentanaBusquedaMovOficina, {
      disableClose: true,
      data: '',
      width: '300px',
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.servicio.cargaMovOficinaId(id)
          .then(datos => {
            if (datos.status === 'error') {
              this._modalMsg.openModalMsg<ModalMsgComponent>(ModalMsgComponent, { data: datos.data }, false, '300px', 'exito')
              return
  ...
  async eliminaroMovInversion(id: number) {
    ...
    const dialogRef = this._dialog.open(VentanaEliminaMovOficina, {
      disableClose: true,
      data: '',
      width: '350px'
    })
      ...
      if (result) {
        this.servicio.GetCredenciales()
          .then(credenciales => {
            ...
            this.servicio.eliminaOficina(id, '0', credenciales.Id)
              .then(respuesta => {
