...
import { Proveedores } from '../../../core/services/proveedores.service';
...
import { VentanaEliminaProveedor } from './ventanaEliminarProveedor';
import { VentanaVerInformacionProveedor } from './ventanaVerInformacionProveedor';
import { VentanaBusquedaProveedor } from './ventanaBusquedaProveedor';
...
export class ProveedoresComponent implements OnInit{
  ...
  setDataLogin() {
    this.puenteData.disparadorData.emit({ dato: 'Proveedores', poisionX: '' })
  }
  ...
  inputBusqueda(event:any){
    ...
    async busqueda(){
      ...
      async verDatosProveedor( id:number ){
        ...
        const dialogRef = this._dialog.open( VentanaVerInformacionProveedor, {
          disableClose:true,
          data:datos,
          width:'705px',
          maxWidth: '100%'
        ...
        editarProveedor( id:number ){
          ...
          const dialogRef = this._dialog.open( VentanaBusquedaProveedor, {
            disableClose:true,
            data: '',
            width: '300px',
        ...
        cargaFormularioProveedor( formProveedor:Array<any> ){
          ...
          this.cargaCuentaTargeta( formProveedor[0].CLABE, formProveedor[0].Banco_cuenta, formProveedor[0].tarjeta, formProveedor[0].Banco_Tarjeta )

        }


        cargaCuentaTargeta( _Clave:string, _BancoCuenta:string, _tarjeta:string, _BancoTargeta:string ){
          if( _Clave != '' ){
          ...
          async eliminarProveedor( id:number ){
            ...
            const dialogRef = this._dialog.open( VentanaEliminaProveedor, {
              disableClose:true,
              data:'',
              width:'300px'
            } )
